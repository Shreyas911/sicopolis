!> @file
!!
!! Fortran9X adaptation of the functionality of Revolve
!! (Griewank and Walther, 2000, doi: 10.1145/347837.347846)
!<
...
ourFirstUTurned, chkRange, forwdCount
  ...
  !! see also `errorMsgRevolve` in \ref rvAction
...
!--------------------------------------------------------------------*
  ...
  !! @param errorMsgRevolve set when an error condition occurs; else set to `"none"`
  !! @param anActionInstance  if supplied initializes its contents
  !! @param bundle if supplied initializes `ourBundle`
  !! @return `.true.` if successfull, else `.false.` ansd `errorMsgRevolve` will be set
  FUNCTION rvInit(steps,checkpoints,errorMsgRevolve,anActionInstance,bundle)
    IMPLICIT NONE
    LOGICAL :: rvInit
    ...
    IF (ourBundle<1 .OR. ourBundle>steps) THEN
       rvInit=.FALSE.
       print *, "ERROR:: ourBundle is ", ourBundle, " steps is ", steps
       errorMsgRevolve = "revolve::rvInit: bundle parameter out of range [1,steps]"
    ELSEIF (steps<0) THEN
       rvInit=.FALSE.
       errorMsgRevolve = 'revolve::rvInit: negative steps'
    ELSEIF (checkpoints<0) THEN
       rvInit=.FALSE.
       errorMsgRevolve = 'revolve::rvInit: negative checkpoints'
    ELSE 
       ourCStart       = 0
       ourSteps        = steps
       ...
       IF (ourVerbosity>0) THEN
          ...
          IF (predFwdCnt==-1) THEN
             errorMsgRevolve='revolve::rvInit: error returned by  revolve::forwdCount'
             rvInit=.FALSE.
             RETURN
          ELSE
             WRITE (*,'(A)') 'prediction:'
...
!--------------------------------------------------------------------*
  ...
  !! @param errorMsgRevolve set when an error condition occurs; else set to `"none"`
  !! @return `.true.` if successfull, else `.false.` ansd `errorMsgRevolve` will be set
  FUNCTION rvAdjust(steps,checkpoints,errorMsgRevolve)
    IMPLICIT NONE
    LOGICAL :: rvAdjust
    INTEGER, INTENT(IN) :: steps
    INTEGER, INTENT(IN) :: checkpoints
    ...
    IF ((ourCEnd-ourCStart)==0) THEN
     ...
     ELSE IF (rwcpTest) THEN
        ...
        IF (ourRWCP+1>ourACP) THEN
           ...
           rvNextAction%errorMsgRevolve='revolve::rvNextAction: insufficient allowed checkpoints'
           RETURN
        ELSE
