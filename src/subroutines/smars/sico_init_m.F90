...
errormsg = ' >>> sico_init: GRID==2 not allowed for smars application!'
call error(errormsg)
...
write(10, fmt=trim(fmt2a)) 'CHASM =', CHASM
#if (CHASM==2)
...
if (ios /= 0) then
   errormsg = ' >>> sico_init: Error when opening the chasm mask file!'
   call error(errormsg)
end if
...
errormsg = ' >>> sico_init: SEA_LEVEL==3 not allowed for smars application!'
call error(errormsg)
...
if (ios /= 0) then
   errormsg = ' >>> topography1: Error when opening the mask file!'
   call error(errormsg)
end if
...
do j=0, JMAX
   ...
   if (maske(j,i) <= 1_i1b) then
      ...
      errormsg = ' >>> topography1: ' &
                    //'maske(j,i)>=2 not allowed for initial topography!'
      call error(errormsg)
   end if
