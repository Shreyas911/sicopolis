...
\begin{large}December 30, 2019\end{large}
...
\noindent{}Copyright 2009--2019 Ralf Greve\\
...
\item \textbf{Option 1: Using subversion [recommended]}
...
\\[-1ex]
\hspace*{6.67mm}\verb+https://swrepo1.awi.de/svn/sicopolis/tags/version51 sicopolis+
\\
...
\item \textbf{Option 2: Downloading a tarball}

Download the gzipped tar archive sicopolis\_v51.tgz from the SICOPOLIS web page (http://www.sicopolis.net/).

Unpacking:

\hspace*{10mm}\verb+tar -x -v -z -f sicopolis_v51.tgz+
...
\\
\phantom{$\longrightarrow$} resolution 25~km, $t=0\ldots{}200\,\mathrm{ka}$ \citep{payne_etal_00}.
\\
...
\\
\phantom{$\longrightarrow$} polythermal two-layer scheme (POLY) \citep{greve_blatter_16}.
...
\\
\phantom{$\longrightarrow$} \citet{greve_etal_17a}].
...
\\
\phantom{$\longrightarrow$} ISMIP6 InitMIP; \citet{greve_etal_17a}].
...
\\
\phantom{$\longrightarrow$} \citep{bernales_etal_17} and ice shelves, resolution 64~km,
\\
...
\\
\phantom{$\longrightarrow$} [similar to \citeauthor{dunse_etal_11}'s (\citeyear{dunse_etal_11}) Exp.~2 (steady fast flow) and
\\
...
\\
\phantom{$\longrightarrow$} [steady-state runs by \citet{greve_07b}].
...
\\
\phantom{$\longrightarrow$} [similar to run nt012 by \citet{greve_etal_99a}].
...
\\
\phantom{$\longrightarrow$} resolution 50~km, $t=0\ldots{}200\,\mathrm{ka}$ \citep{calov_etal_10}.
...
\item SICOGRAPH is included in the subversion repository of SICOPOLIS. You'll find it in the directory sicopolis/tools/sicograph\,.
...
\end{tabular}
\caption{Model times, time steps and computing (CPU) times for the EISMINT, Greenland and Antarctica simulations contained in the script multi\_sico\_1.sh, run with SICOPOLIS V5.1 and the Intel Fortran Compiler 17.0.1 for Linux (optimization options -xHOST -O3 -no-prec-div) on a $2\times{}14$-Core Intel Xeon E5-2690~v4 (2.6~GHz) PC under openSUSE Leap 42.3.
\protect\\
...
\bibliography{journals2_ralf,Refs_for_Quick_Start}
