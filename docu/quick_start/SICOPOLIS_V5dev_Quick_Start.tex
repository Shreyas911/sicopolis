...
For the \textbf{Intel compiler}, manual installation is required. The C and Fortran libraries are available for downloaded on the NetCDF website as zip or tar archives. Unzip them into temporary directories. 

Prior to version 4.2, a single archive contained both the C and Fortran libraries. A minimal installation for version 4.1.3 can be done as follows:

\begin{verbatim}
./configure --prefix=/opt/netcdf --libdir=/opt/netcdf/lib \
            --disable-netcdf-4 \
...
Installation under /opt usually requires admin rights. For a local installation, replace `/opt' (in both -{}-prefix and -{}-libdir) by `/home/$<$my\_user\_name$>$/local'.

Since version 4.2, the C and Fortran libraries must be installed separately. See the NetCDF website for further instructions.
...
Download the latest version of Lis as a zip archive from https://www.ssisc.org/lis/ (as of January 23, 2021: lis-2.0.30.zip). Unzip the archive into a temporary directory. For \textbf{GCC}, install lis by executing:

\begin{verbatim}
./configure --prefix=/opt/lis --enable-fortran --enable-f90 \
