...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   real(dp),     dimension(7)        :: fc_collected
   real(dp),     dimension(7)        :: direction
...
!@ python_automated_grdchk_scalar IO begin @
          ...
          ! Calculate 4-order acccurate finite difference gradient
          ! Don't perturb in negative direction if orig_val is 0.0
          if (orig_val .ne. 0) then
            gfd = (-fc_collected(4) + 8*fc_collected(2) - 8*fc_collected(3) + fc_collected(5)) / (12.0 * perturb_val * orig_val)
          else
            gfd = (-25*fc_collected(1) + 48*fc_collected(2) - 36*fc_collected(4) + 16*fc_collected(6) - 3*fc_collected(7)) / (12.0 * perturb_val)
          end if          
          ...
          write(98, fmt='(f40.20)') fc_collected(4)
          write(98, fmt='(f40.20)') fc_collected(5)
          write(98, fmt='(f40.20)') fc_collected(6)
          write(98, fmt='(f40.20)') fc_collected(7)
          write(98, fmt='(a)') '----------------------------------'
